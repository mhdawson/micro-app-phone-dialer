...
Server.getTemplateReplacments = function() {
  ...
  for (i = 0; i < Server.config.dialOptions.length; i++) {
    ...
    dialerOptions[i] = '<tr><td><button style="width:100%" onclick="startRequest(\'' + phone_request + '\',' + i + ')">' + Server.config.dialOptions[i].title  + '</button></td></tr>';
    height = height + HEIGHT_PER_ENTRY;
  }
  dialerOptions[Server.config.dialOptions.length] = '<tr><td id="status">Done</td></tr>';
  height = height + HEIGHT_PER_ENTRY;
...
Server.startServer = function(server) {
  var phone = new nodecti({'hostname': Server.config.phoneIP});
